...
\begin{minipage}[t]{\dimexpr\textwidth-5cm}
 ...
 \[
  f_a(x) = \textellipsis
 \]
 Im Taschenrechner kann dies als 
 \[
  \texttt{f(x,a) := \textellipsis} 
 \] 
...
\begin{minipage}[t]{\dimexpr\textwidth-5cm}
 ...
 \item[3.] Finden der Ortskurvenfunktion $t(x)$. Es gilt bereits $t(p_x) = f_a(p_x)$ mit $p_x$ in Abhängigkeit von $a$, die Ortskurvenfunktion $t(x)$ soll aber nur von $x$ abhängen. Dies kann erreicht werden, indem ein $a$ in Abhängigkeit von $x$
 \end{enumerate}
 \end{minipage}
\hfill
\begin{minipage}[t]{5cm}
...
\begin{enumerate}
 \vspace{-0.7em} 
 \item[] gefunden und eingesetzt wird. Gefunden werden kann dieses, indem $x = p_x$ nach $a$ aufgelöst wird. Dieses $a$ kann in das bereits bestimmte $t(p_x) = f_a(p_x)$ eingesetzt werden, sodass eine von $x$ abhängige Funktion, welche die Ortskurve beschreibt rauskommt. Diese kann als $t(x)$ genutzt werden.
\end{enumerate}
...
\begin{enumerate}
 ...
 \item Wird das gefundene $p_x$ in das gegebene $f(x)$ eingesetzt, so ist die gesamte Koordinate eines Minimum immer beim Punkt $\left(-0.5a \,\middle|\, \left(-0.5a\right)^2-0.5a \cdot a\right)$
\end{enumerate}
...
\noindent \begin{minipage}[t]{\dimexpr\textwidth-5cm} 
 ...
 Das auflösen von $p_x$ in Abhängigkeit von  $a$ gefolgt von $a$ in Abhängigkeit von $x$ mag wie ein unnötiger Schritt, welcher übersprungen werden kann, scheinen, ist dies aber nicht. Bei komplexeren Funktionen sind oftmals nach naivem auflösen die $x$-Werte beide Funktionen und des Punktes auf unterschiedlichen skalen.
\end{minipage} 
\hfill 
\begin{minipage}[t]{5cm} 
 \centering
