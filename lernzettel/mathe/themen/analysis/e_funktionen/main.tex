...
\begin{minipage}{5cm} 
 \centering
 \begin{tikzpicture}     
  \draw[->, blue, thick, domain=-2:1.3, samples=100] 
            plot (\x, {e^(\x)});
  ...
  \draw (0, 1) node {$\times$} node[left] {(0 $\vert$ 1)};
  \draw (1, e) node {$\times$} node[right] {(1 $\vert$ e)};
...
\centering
 \begin{tikzpicture}     
  \draw[->, blue, thick, domain=e^(-3):3, samples=100] 
            plot (\x, {ln(\x)});
  ...
  \draw (0, 0) node[below left] {0}; 
  \draw (e, 1) node {$\times$} node[above left] {(e $\vert$ 1)};
  \draw (1, 0) node {$\times$} node[below right] {(1 $\vert$ 0)};
