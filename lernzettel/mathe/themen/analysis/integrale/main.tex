...
\subsection{Bestände} 
Gibt die Funktion $f(x)$ eine Veränderung eines Bestandes mit einer Einheit an so ist die Einheit der Stammfunktion die Einheit von der Einheit von $f(x)$, multipliziert mit der Einheit von $x$. Somit ist die Veränderung des Bestandes zwischen Zeitpunkt $a$ und $b$ dann
\[
 \int_a^b f(x) \dx
\]
