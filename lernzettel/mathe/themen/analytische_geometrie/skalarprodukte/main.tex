...
\subsection{Als Projektion}
\begin{minipage}{\dimexpr\linewidth-3cm}
Das Skalarprodukt kann aber auch als Projektion angesehen werden. Dabei wird der eine Vektor von oben auf den anderen projiziert. Diese Länge wird dann mit der Länge des anderen Vektors multipliziert.
\[
...
\begin{tikzpicture}
 ...
 \draw (1, 0) arc[start angle=0,end angle=45,radius=1];
 \draw ({0.6*sin(67.5)},{0.6*cos(67.5)}) node {$\alpha$};
 \draw[thick, dashed] ({2*sin(45)},0) -- ({2*sin(45)},{2*cos(45)});
 
 \draw[thick,green!50!black] (0,-0.2) -- ({2*sin(45)}, -0.2) node[below, pos=0.5] {$\norm{\vect{b}} \cdot \cos(\alpha)$};
 \draw[thick,green!50!black] (0,-0.1) -- (0, -0.3);
 \draw[thick,green!50!black] ({2*sin(45)},-0.1) -- ({2*sin(45)}, -0.3);
\end{tikzpicture}  
\end{minipage} 
\end{document}