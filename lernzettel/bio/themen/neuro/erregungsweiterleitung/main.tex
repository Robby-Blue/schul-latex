...
Dass das Rezeptorpotential zum Axon weitergeleitet wird ist, sollte es stark genug sein, gegeben. Im Innenraum einer Zelle werden Potentiale \emph{passiv} weitergeleitet, ohne dass etwas weiteres passieren muss. Dabei nehmen sie aber mit der Distanz an stärke ab.
 
Damit das entstandene Aktionspotential über das Axon bis zur Synapse weitergeleitet werden kann, muss ein anderer, aktiver, prozess geschehen.
 
\subsection{Ablauf} 
Wird der Schwellenwert von $-50\,\text{mV}$ an einem bestimmten Punkt im Axon erreicht so öffnen sich dort spannungsgesteuerte \ce{Na+}-Ionenkanäle, \ce{Na+}-Ionen strömen in das Axon. Weiter entlang des Axons liegen negative Ladungen, welche von den positiven Ladung der Depolarisation angezogen werden und diese anziehen. Dies sind die Ausgleichsströme. Aufgrund dieser kommt es am nächsten Ionenkanälen zu einer eigenen Depolarisation, weshalb sich die Ionenkanäle öffnen und der Zyklus sich wiederholen kann.
...
\begin{description}
 ...
 \item[saltatorische Erregungsleitung] liegt vor, wenn am Axon zwischen den Stellen mit Ionenkanälen, den Ranvierschen Schürringen, auch Myelinscheiden vorliegen. An diesen liegen keine Ionenkanäle und minimierte wirkende Spannungen zwischen Ladungen innerhalb und außerhalb des Axons vor, mit der Myelinscheide als Isolator wirkend. Somit muss, beziehungsweise kann, an diesen Stellen kein Aktionspotential ausgelöst werden, und die elektrische Erregung \textquote{überspringt} diese Stellen sozusagen.
\end{description} 
