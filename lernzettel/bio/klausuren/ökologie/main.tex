...
\begin{figure}[h] 
  \centering
  \begin{tikzpicture}     
    \draw[thick] plot[smooth, tension=1] coordinates {(-2.5,0) (-1, 2) (1, 4.5) (2, 2) (3, 0)};
     
    \draw[->] (-5,0) -- (5,0) node[below left] {Umgebungstemperatur};
    \draw[->] (-5,0) -- (-5,5) node[right] {Körpertemperatur}; 
    
    \fill[red!30, opacity=0.5] (-4.3, 0) rectangle (-2.5, 4.5);    
    \fill[red!30, opacity=0.5] (3, 0) rectangle (4.3, 4.5);
 
    \node[] at (-3.4, 3) {Kältestarre}; 
    \node[] at (3.65, 3) {Hitzestarre};
 
    \draw[red] (-4.3, 0) node {$\times$} node[above] {Kältetod};    
    \draw[red] (4.3, 0) node {$\times$} node[above] {Hitzetod}; 
  \end{tikzpicture}
\end{figure} 
 
\subsection{Thermoregulierer}
Thermoregulierer regulieren ihre eigene Körpertemperatur durch Aktionen wie Zittern, Schwitzen, Hecheln, etc. Dies benötigt viel Energie, erlaubt ihnen aber auch eine konstante Körpertemperatur haben können (sie sind \textbf{Homoiotherm}). Somit können sie überall gut leben, auch in kalten Regionen. \newline
Homoiotherme Tiere, welche aktiv Energie verwenden müssen um sich warm zu halten, sind von der \textbf{Bergmannschen Regel} und der \textbf{Allenschen Regel} betroffen, welche sich mit der Produktion, Speicherung und Abgabe von Wärme in bezug auf (Körper-)größen bezieht. Poikilotheme Tiere sind davon einfach nicht betroffen.
 
\begin{figure}[h] 
  \centering
  \begin{tikzpicture}     
    \draw[thick] plot[smooth, tension=0.2] coordinates {(-4.3,0) (-3.5, 4.5) (3.5, 4.5) (4.3, 0)};
